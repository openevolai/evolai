...
if __name__ == "__main__":
    ...
    setup_logging(
        ...
        use_json=True,
        extra_fields={"validator_id": "validator-001", "netuid": 47}
    )
    
    # Use structured logger
    logger = get_structured_logger(__name__)
    
    logger.info("Validator started", version="0.2.0", gpu_count=1)
    logger.warning("High GPU usage detected", gpu_usage_percent=85.3, threshold=80.0)
    
    try:
        raise ValueError("Example error")
    except Exception as e:
        logger.error("Operation failed", exc_info=True, operation="model_load", model_name="test/model")
