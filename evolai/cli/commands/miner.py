"""Miner commands for model registration and eligibility checking"""
...
def check_model_eligibility(model_name: str, revision: str = "main", track: str = "transformer") -> dict:
    """
    Check if a model meets eligibility requirements
    
    Args:
        model_name: HuggingFace model name
        revision: Git revision (branch, tag, or commit SHA)
        track: Competition track (transformer or mamba2)
    ...
    # Check model name includes 'evolai' prefix
    model_name_lower = model_name.lower()
    # Extract just the model name part (after the /)
    if "/" in model_name:
    ...
    try:
        ...
        with Progress(
            ...
            # SECURITY: trust_remote_code executes code bundled in the model repo.
            # Only run this against models you trust or in an isolated environment.
            ...
            # Download model to properly check eligibility
            ...
            # Check if it has required methods (without loading full model)
            architecture = config.model_type if hasattr(config, 'model_type') else "unknown"
            info["architecture"] = architecture
            
            # Check track-specific requirements
            if track == "mamba2":
                # Mamba2 should have mamba/recurrent/stateful architecture
                ...
                # Standard transformer - check vLLM compatibility
                # Based on vLLM supported models: https://docs.vllm.ai/en/latest/models/supported_models/#text-generation
                vllm_architectures = [
            ...
            # Check HuggingFace interface compatibility
...
def check_model(
    ...
    """
    Check if your model meets eligibility requirements
    
    This command validates:
    - Model name includes 'evolai' (e.g., username/evolai-model-9b)
    - Model size: 0.45-0.48B, 1.5-1.8B, 3.5-3.8B, 9-9.5B, or 21-21.5B
    - Architecture compatibility (track-specific)
    - HuggingFace interface implementation
    
    Examples:
        evolcli miner check
        evolcli miner check myorg/evolai-transformer-9b
        evolcli miner check myusername/evolai-model-9b --revision v1.0
        evolcli miner check myorg/evolai-mamba2-9b --track mamba2 --revision main
    """
...
def register_model(
    ...
    """
    Register your model on the Bittensor subnet
    
    This command will:
    1. Verify HuggingFace model ownership
    2. Register model metadata on-chain
    ...
    Examples:
        ...
        # With arguments
        evolcli miner register myorg/evolai-model-9b --wallet-name miner1
        evolcli miner register username/evolai-transformer-9b --wallet miner1 --revision v1.0
        evolcli miner register myorg/evolai-mamba2-9b --wallet miner1 --track mamba2 --revision main
    """
    # Interactive prompts if arguments not provided
    ...
    # Ask for model name based on track choice
    if model_name is None:
    ...
    # Get available hotkeys for the wallet
    if hotkey is None or hotkey == "default":  # Prompt if not provided or is default
    ...
    # Single track registration (miners can only register for one track)
...
def _register_single_track(
    ...
    """Helper function to register a single track"""
    ...
    try:
        ...
        # Create metadata (single track only - no preservation needed)
        metadata = {
            track: {
        ...
        # Compress metadata to fit in 128 byte limit
        ...
        # Store full metadata locally for reference
        ...
        # Commit compressed metadata to chain
...
def show_info():
    """
    Show information about miner requirements and competition tracks
    """
    info_text = """
...
def get_challenge(
    ...
    UID is your numeric position in the subnet metagraph (an integer).
    Find it with: btcli subnets metagraph --netuid 47
    ...
    if not api_url.startswith(("https://", "http://")):
        err_console.print("api_url must use https:// or http:// scheme")
        raise typer.Exit(1)

    console.print(f"[cyan]Fetching challenge for UID {uid}...[/cyan]")

    try:
        resp = httpx.get(f"{api_url}/challenge/{uid}", timeout=15)
    except httpx.ConnectError:
        console.print("[red]✗ Could not connect to the challenge service[/red]")
        raise typer.Exit(1)

    if resp.status_code == 404:
        console.print(f"[yellow]⚠ No challenge found for UID {uid}[/yellow]")
        raise typer.Exit(1)

    resp.raise_for_status()
    data = resp.json()

    table = Table(title=f"Challenge for UID {uid}", show_header=True, header_style="bold cyan")
    ...
    if datasets:
        ...
        # back-compat with old single-dataset format
        table.add_row("Dataset", data.get("dataset_name", "unknown"))
        _idx = data.get("text_indices", [])
        table.add_row("# Indices", str(len(_idx)))
        table.add_row("Indices (first 10)", str(_idx[:10]))
